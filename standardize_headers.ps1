...
# This script updates all HTML pages to use the standard header from home.html

Write-Host "=== ALT MATE Header Standardization Script ===" -ForegroundColor Cyan
Write-Host ""

# Define the standard header (from home.html)
$standardMobileHeader = @'
    ...
    <div class="mobile-sidebar" id="mobileSidebar">
        <div class="sidebar-header">
            <img src="../images/tiraz%20logo.png" alt="ALT MATE Logo" style="height: 48px;">
...
Write-Host "This script will update headers for:" -ForegroundColor Yellow
Write-Host "1. Main pages (aboutus, missions, services, bundles, samples, contact, request-service, blog)" -ForegroundColor White
Write-Host "2. All service detail pages (19 pages in services/ folder)" -ForegroundColor White
Write-Host ""
Write-Host "Standard header includes:" -ForegroundColor Green
Write-Host "  ✓ Mobile hamburger menu" -ForegroundColor White
Write-Host "  ✓ Mobile sidebar with all navigation links" -ForegroundColor White
Write-Host "  ✓ Desktop navigation with: Home, About Us, Missions, Services, Bundles, Samples, Blog" -ForegroundColor White
Write-Host "  ✓ Contact Info button" -ForegroundColor White
Write-Host ""

Write-Host "NOTE: You'll need to manually update each file or use find/replace in VS Code" -ForegroundColor Cyan
Write-Host ""
Write-Host "Instructions:" -ForegroundColor Yellow
Write-Host "1. For each page, replace the content from <body> to </header> with the standard header" -ForegroundColor White
Write-Host "2. Adjust 'active' class to match the current page" -ForegroundColor White
Write-Host "3. For service pages, adjust paths (../ to ../../)" -ForegroundColor White
Write-Host ""
...
foreach ($page in $mainPages) {
    Write-Host "  [ ] html/$page"
}

Write-Host ""
Write-Host "Service Pages:" -ForegroundColor Green
foreach ($page in $servicePages) {
    Write-Host "  [ ] html/$page"
}

Write-Host ""
Write-Host "Would you like me to show you the exact code to use? (y/n)" -ForegroundColor Yellow
$response = Read-Host

if ($response -eq 'y' -or $response -eq 'Y') {
    Write-Host ""
    Write-Host "=== STANDARD HEADER FOR MAIN PAGES ===" -ForegroundColor Cyan
    Write-Host $standardMobileHeader
    Write-Host ""
    Write-Host "Note: Save this to a text file for easy reference!" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "Done! Good luck with the updates! 🚀" -ForegroundColor Green
