# PowerShell script to update all service page headers with the standardized header from home.html
# This script adds the mobile hamburger menu, sidebar navigation, and Blog link to all service pages

# Define the list of service pages to update (excluding web-development.html which is already done)
$servicePages = @(
...
# Define the old header pattern (what needs to be replaced)
$oldHeader = @'
...
# Define the new header (includes hamburger menu, sidebar, and Blog link)
$newHeader = @'
...
# Base path for service pages
$basePath = ".\html\services\"

# Counter for successful updates
$updatedCount = 0

Write-Host "Starting header update for service pages..." -ForegroundColor Cyan
Write-Host "Total pages to update: $($servicePages.Count)" -ForegroundColor Yellow
Write-Host ""
...
foreach ($page in $servicePages) {
    ...
    if (Test-Path $filePath) {
        try {
            ...
            # Check if the old header exists in the file
            if ($content -match [regex]::Escape($oldHeader)) {
                ...
                Write-Host "⚠ Skipped: $page (old header pattern not found)" -ForegroundColor Yellow
            }
        } catch {
...
Write-Host "Successfully updated: $updatedCount out of $($servicePages.Count) pages" -ForegroundColor Green
Write-Host ""
Write-Host "Summary:" -ForegroundColor Cyan
Write-Host "- All updated service pages now have:" -ForegroundColor White
Write-Host "  - Mobile hamburger menu button" -ForegroundColor Gray
Write-Host "  - Mobile sidebar navigation with icons" -ForegroundColor Gray
Write-Host "  - Blog link in navigation" -ForegroundColor Gray
Write-Host "  - Bundles link in navigation" -ForegroundColor Gray
Write-Host "  - Social media icons in sidebar footer" -ForegroundColor Gray
